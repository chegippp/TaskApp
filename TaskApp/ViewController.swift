...
//  taskApp
//
//  Created by y.hara on 2025/01/15.
//
...
class ViewController: UIViewController, UITableViewDelegate, UISearchBarDelegate, UITableViewDataSource {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        if searchBar.text == "" {
            ...
            // 検索条件に該当するカテゴリーのデータを取得
            taskArray = try! Realm().objects(Task.self).sorted(byKeyPath: "date", ascending: true).where({$0.category == searchBar.text!})
        }
        // キーボードを閉じてTableViewを更新
